 ...
 * Sends booking confirmation email via Resend API (key stays server-side)
 *
 * Body: {
 *   attendee_name, attendee_email, event_title, event_date,
 *   event_venue, event_city, booking_id, ticket_qr_code,
 *   amount_paid, organizer_name
 * }
 */
router.post('/send-confirmation', async (req, res) => {
  try {
    ...
    if (!attendee_email || !event_title || !booking_id) {
      return res.status(400).json({ error: 'Missing required fields: attendee_email, event_title, booking_id' });
    }
    ...
    const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(ticket_qr_code || booking_id)}&bgcolor=ffffff&color=0a0f1e`;
...
<body style="margin:0;padding:0;background:#0a0f1e;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0f1e;padding:32px 0;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#111827;border-radius:12px;border:1px solid #2a3a55;">
                <tr>
                  <td style="padding:16px 20px;">
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td style="font-size:13px;color:#8899bb;">Amount Paid</td>
                        <td align="right" style="font-size:16px;color:#f0f4ff;font-weight:700;">${formattedAmount}</td>
                      </tr>
                      <tr>
                        <td colspan="2" style="padding-top:8px;">
                          <table width="100%" cellpadding="0" cellspacing="0">
                            <tr>
                              <td style="font-size:13px;color:#8899bb;padding-top:8px;">Status</td>
    ...
    console.log(`[Email] Confirmation sent to ${attendee_email} — ID: ${resendData.id}`);

    res.json({ success: true, id: resendData.id });
  } catch (err) {
    console.error('[Email] send-confirmation error:', err.message);
